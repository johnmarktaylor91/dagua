#!/bin/bash
# Overnight: finish benchmark retry + run post-benchmark pipeline + notify.
# Chained so one dispatch call covers the whole overnight flow.
#
# Steps:
#   1. run_benchmark.py --resume with MAX_INFLIGHT_GROUPS=4 (patched in-file),
#      workers=2, watchdog 600s. Picks up ~8.3k running + ~42.9k cleared
#      watchdog rows + ~22k remaining scope.
#   2. post_benchmark_pipeline.sh (verify 0 running, HDF5 consolidate,
#      fidelity pipeline, quality/runtime pipeline).
#   3. iMessage notification of overall outcome.
#
# All stdout/stderr captured by the parent dispatch.sh log.
set -uo pipefail
...
echo "=== [overnight] step 1: benchmark --resume ==="
...
BENCH_EXIT=$?
echo "=== [overnight] benchmark exited with $BENCH_EXIT at $(date -Iseconds) ==="

if [ $BENCH_EXIT -ne 0 ]; then
    echo "[overnight] Benchmark failed -- skipping post pipeline"
    ~/.claude/scripts/send-to-jmt.sh "Dagua overnight FAILED: benchmark exit=$BENCH_EXIT. No post-pipeline run. See log at .project-context/tasks/overnight-finish.log" || true
    exit $BENCH_EXIT
fi

# Sanity: how many running rows remain?
REMAIN=$(python3 -c "
import json
...
echo "=== [overnight] step 2: post-benchmark pipeline ==="
./scripts/post_benchmark_pipeline.sh
POST_EXIT=$?
echo "=== [overnight] post-pipeline exited with $POST_EXIT at $(date -Iseconds) ==="
