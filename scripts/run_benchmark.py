...
# Provenance marker prefix for rows rebuilt from on-disk tensors rather than
# executed by this process.  Recovery cannot know which code state produced a
# tensor found on disk, so recovered rows must never carry a bare current sha
# (that would claim the current HEAD generated bytes it never produced).
RECOVERED_GIT_SHA_PREFIX = "recovered:"
...
def effective_worker_capacity(
    resolved_workers: int,
    zombie_futures: set[Future[list[dict[str, Any]]]],
) -> int:
    """Return how many pool workers can actually execute new work.

    A live zombie (watchdog-expired future whose worker is still stuck in
    its call) occupies one executor worker until pool teardown. Watchdog
    bookkeeping must therefore treat only the first
    ``resolved_workers - live_zombies`` visible inflight futures as
    actively executing: a queued future behind a zombie cannot run, so
    starting (or expiring) its watchdog timer would record a false
    watchdog error for work that never executed.
    ...
    zombie_futures : set[Future[list[dict[str, Any]]]]
        Registry of expired-but-possibly-still-running futures. Entries
        whose call eventually returned (``done()``) free their worker and
        no longer reduce capacity.

    Returns
    -------
    int
        Worker slots available for queued work, floored at ``1``. The
        floor is defensive only: the last live non-zombie active future
        expiring makes ``active_expired`` true in the same watchdog event,
        which rebuilds the pool and clears the zombie registry, so
        capacity cannot legitimately sit at zero across scheduler
        iterations.
    """
    live_zombies = sum(1 for fut in zombie_futures if not fut.done())
    return max(1, resolved_workers - live_zombies)


def drain_inflight_bounded(
    inflight: dict[Future[list[dict[str, Any]]], Sequence["WorkItem"]],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--retry-watchdog-errors",
        action="store_true",
        help=(
            "With --resume, treat rows previously recorded as watchdog errors "
            "(error starting with 'watchdog:') as incomplete so they are retried. "
            "Without this flag, resume keeps error rows permanent (historical "
            "behavior)."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Allow a non-resume run to overwrite results.json in an output "
            "directory whose existing results.json contains rows OUTSIDE the "
            "current graph/engine scope. Without this flag such runs abort, "
            "because a non-resume run drops all out-of-scope rows from "
            "results.json while leaving their position tensors orphaned."
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def recover_results_from_positions(
    ...
    git_sha : str
        Current git commit SHA. Recovered rows are stamped with a
        ``recovered:``-prefixed marker built from it, never the bare sha:
        a tensor found on disk has unknown provenance and must not be
        attributed to the current code state.
    ...
    for path in positions_dir.glob("*.pt"):
        ...
        record = _record_with_pairings(
            ...
            skip_reason=None,
            git_sha=f"{RECOVERED_GIT_SHA_PREFIX}{git_sha}",
        )
...
def merge_recovered_results(
    ...
    -------
    dict[str, BenchmarkRecord]
        Merged results. Recovered rows only fill in keys that have no
        executed outcome: keys missing from ``existing_results`` or stuck in
        ``running`` state (crash mid-flight). Existing ``ok`` rows keep their
        genuine provenance and runtime; existing ``error``/``timeout`` rows
        are never silently self-healed into successes by orphan tensors.
    """
    merged = dict(existing_results)
    for key, recovered_record in recovered_results.items():
        existing_record = merged.get(key)
        if existing_record is not None and existing_record.status != RUNNING_STATUS:
            continue
...
def is_record_complete(
    ...
    output_dir: Path,
    save_positions: bool,
    retry_watchdog_errors: bool = False,
) -> bool:
    ...
    retry_watchdog_errors : bool, default=False
        When ``True``, rows recorded as watchdog errors (``error`` starting
        with ``"watchdog:"``) are treated as incomplete so resume retries
        them. Default preserves the historical error-rows-are-permanent
        semantics.
    ...
    if record is None or record.status == RUNNING_STATUS:
        return False
    if (
        retry_watchdog_errors
        and record.status == "error"
        and str(record.error or "").startswith("watchdog:")
    ):
        return False
...
def serial_watchdog_warning(
    resolved_workers: int,
    watchdog_timeout: Optional[float],
) -> Optional[str]:
    """Return a warning when ``--watchdog-timeout`` is inert for this run.

    The worker-pool watchdog only exists on the parallel execution path;
    with ``--workers 1`` the flag is accepted and recorded but never used,
    leaving only the in-worker SIGALRM (which cannot interrupt a hung
    C-level kernel -- the exact case the watchdog was built for).
    ...
    watchdog_timeout : float | None
        Explicit ``--watchdog-timeout`` value, or ``None`` when defaulted.

    Returns
    -------
    str | None
        Warning message when an explicit watchdog timeout is a no-op,
        otherwise ``None``.
    """
    if watchdog_timeout is None or resolved_workers > 1:
        return None
    return (
        "[benchmark] WARNING: --watchdog-timeout has NO EFFECT with --workers 1 "
        "(the watchdog only guards the parallel worker pool). Serial rows are "
        "protected only by the in-worker --timeout alarm, which cannot "
        "interrupt a hung C-level kernel."
    )


def non_resume_out_of_scope_keys(
    disk_results: dict[str, BenchmarkRecord],
    scoped_keys: Sequence[str],
) -> list[str]:
    """Return existing result keys a non-resume run would silently drop.

    A non-resume run rebuilds ``results.json`` from the current scope only,
    so any pre-existing row outside the planned graph/engine/seed scope is
    destroyed on the first save. Callers abort when this list is non-empty
    unless ``--force-scope-rewrite`` was given.

    Parameters
    ----------
    disk_results : dict[str, BenchmarkRecord]
        Rows currently stored in the output directory's ``results.json``.
    scoped_keys : Sequence[str]
        Record keys planned for the current invocation.

    Returns
    -------
    list[str]
        Sorted keys present on disk but absent from the current scope.
    """
    return sorted(set(disk_results) - set(scoped_keys))


def final_exit_code(counts: dict[str, int], for_counts_ok: bool) -> int:
    """Derive the process exit code from final row-status counts.

    Parameters
    ----------
    counts : dict[str, int]
        Status counts over the scoped records.
    for_counts_ok : bool
        Whether the seeded-reference ``__for__`` row-count check passed.

    Returns
    -------
    int
        ``0`` only when the reference row-count check passed and no row
        errored or timed out; ``1`` otherwise, so automation cannot mistake
        a run with failed rows for a clean regen.
    """
    if not for_counts_ok:
        return 1
    if counts.get("error", 0) + counts.get("timeout", 0) > 0:
        return 1
    return 0
...
def main() -> int:
    ...
    watchdog_warning = serial_watchdog_warning(args.resolved_workers, args.watchdog_timeout)
    if watchdog_warning is not None:
        print(watchdog_warning)
        ...
        for competitor in selected_engines:
            for seed in seeds_for_engine(competitor.name, args.seeds, args.seed_start, seed_refs):
                ...
                if is_record_complete(
                    ...
                    retry_watchdog_errors=bool(args.retry_watchdog_errors),
                ):
                    completed_scope += 1
    ...
    if not args.resume and disk_results and not args.force_scope_rewrite:
        dropped_keys = non_resume_out_of_scope_keys(disk_results, scoped_keys)
        if dropped_keys:
            preview = ", ".join(dropped_keys[:5])
            print(
                "[benchmark] ERROR: refusing non-resume run into existing output dir "
                f"{output_dir}: results.json holds {len(dropped_keys)} rows outside "
                f"the current scope (e.g. {preview}). A non-resume run would DROP "
                "those rows and orphan their position tensors. Use --resume, a "
                "fresh --output-dir, or --force-scope-rewrite to overwrite anyway."
            )
            return 2
    ...
    if not work_items:
        ...
        return final_exit_code(counts, for_counts_ok)
        ...
        if args.resolved_workers <= 1:
            ...
            def _fill_inflight() -> None:
                ...
                refresh_watchdog_start_times(
                    inflight,
                    watchdog_started_at,
                    max_active=effective_worker_capacity(args.resolved_workers, zombie_futures),
                    now=time.monotonic(),
                )
            ...
            def _handle_watchdog_timeout() -> None:
                ...
                """
                now = time.monotonic()
                # A live zombie still occupies a worker: only the first
                # effective-capacity visible futures are actually executing,
                # so only they may be timed or expired (a queued future
                # behind a zombie must never be recorded as a watchdog
                # error without having run).
                max_active = effective_worker_capacity(args.resolved_workers, zombie_futures)
                active_futures = list(inflight.keys())[:max_active]
            ...
            try:
                ...
                # with never-expired wedged futures still inflight: cancel the
                # queued ones and register the running ones as zombies so the
                # drain below cannot block forever.
                if _shutdown_requested:
                    for fut in list(inflight):
                        if not fut.done():
    ...
    exit_code = final_exit_code(counts, for_counts_ok)
    if exit_code != 0 and for_counts_ok:
        print("[benchmark] FAILED rows present (error/timeout > 0) -- exit 1")
    return exit_code
