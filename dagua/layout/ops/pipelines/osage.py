"""Deterministic osage-style packing pipeline."""
...
def build_osage_pipeline(scale: float = 1.0) -> Pipeline:
    ...
    """
    return Pipeline(
        [NetworkXSimpleLayout("arc", {"start": 0, "scale": scale})],
        name="osage_pipeline",
    )
...
def layout_osage_pipeline(
    ...
    state = build_osage_pipeline(scale=scale).apply(
        problem,
        SolveState(),
...
__all__ = ["build_osage_pipeline", "layout_osage_pipeline"]
