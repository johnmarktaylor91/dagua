...
class _RandomNodeSet:
    ...
    def delete(self, node: int) -> None:
        ...
        -------
        None
            This method mutates the node set in place.
        """
...
def _build_undirected_adjacency(edge_index: torch.Tensor, num_nodes: int) -> list[list[int]]:
    ...
    """
    weighted_adjacency = _build_weighted_adjacency(edge_index, num_nodes)
    return [[neighbor for neighbor, _ in neighbors] for neighbors in weighted_adjacency]
...
def _hierarchy(
    ...
    """
    levels, steps = _build_hierarchy(edge_index, num_nodes, seed=42)
    return [level.edge_index for level in levels], [step.mapping for step in steps]
...
def layout_fmmm(
    ...
    for level in range(len(steps_per_level) - 1, -1, -1):
        fine_positions = _prolong_positions(positions, steps_per_level[level], rng)
        positions = _refine_level_with_edge_lengths(
            fine_positions,
