...
def test_holdout_manifest_has_hashes_not_edges():
    ...
    for key in forbidden_keys:
        assert key not in text, (
            ...
            f"(found key '{key}'). Use topology_sha256 only."
        )
...
def test_holdout_regeneration_is_deterministic():
    ...
    hashes1 = [e["topology_sha256"] for e in m1.entries]
    hashes2 = [e["topology_sha256"] for e in m2.entries]
...
def test_holdout_rolling_never_collides():
    ...
    holdout_hashes = {e["topology_sha256"] for e in mh.entries}
    rolling_hashes = {e["topology_sha256"] for e in mr.entries}
