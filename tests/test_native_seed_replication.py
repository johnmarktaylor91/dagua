...
def test_frozen_seed_bank_preserves_baseline_and_supports_k1() -> None:
    """The bank is frozen and its forced-k1 prefix is baseline-byte-inert."""
    config = LayoutConfig()
...
def test_seed_family_admission_is_all_or_nothing() -> None:
    """One ledger event admits every replica, while a tight ledger admits none."""
    ...
    assert admit_seed_family(admitted_config, base, "probe", seeds) is True
    ...
    rejected_config = LayoutConfig()
    install_budget_ledger(rejected_config, timeout_s=5.0, safety=0.9)
    assert admit_seed_family(rejected_config, base, "probe", seeds) is False
    rejected_ledger = getattr(rejected_config, "_dagua_native_budget_ledger")
    assert rejected_ledger.spent_dwu == 0.0
    assert [event["event"] for event in rejected_ledger.event_log] == ["skip"]
