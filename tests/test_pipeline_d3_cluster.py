"""Pipeline pins for the d3-hierarchy cluster source port."""
...
from dagua.layout.ops.pipelines.d3_cluster import (
...
def test_d3_cluster_pipeline_and_op_are_registered() -> None:
    """Register d3 cluster pipelines and coordinate op.

    Returns
    -------
    None
        Registry lookups must resolve the new cluster entries.
    """
    assert PIPELINE_REGISTRY["d3_cluster"] == (
        "dagua.layout.ops.pipelines.d3_cluster",
        "layout_d3_cluster_pipeline",
    )
    assert PIPELINE_REGISTRY["d3_cluster_radial"] == (
        "dagua.layout.ops.pipelines.d3_cluster",
        "layout_d3_cluster_radial_pipeline",
    )
    assert get_pipeline_function("D3_CLUSTER") is layout_d3_cluster_pipeline
    assert get_op_class("d3_cluster_layout").__name__ == "D3ClusterLayout"


def test_d3_cluster_pipeline_has_stage_composition() -> None:
    """Pin the d3 cluster implementation as a composable op pipeline.
    ...
    assert [operation.name for operation in pipeline.ops] == ["d3_cluster_layout"]


def test_d3_cluster_binary_position_pin() -> None:
    """Pin d3 cluster size coordinates for a binary tree.

    Returns
    -------
    None
        Coordinates must match d3 ``cluster()`` exactly.
    """
    edge_index = _edge_index([(0, 1), (0, 2), (1, 3), (1, 4), (2, 5), (2, 6)])
    positions = layout_d3_cluster_pipeline(edge_index, 7)
...
def test_d3_cluster_matches_reference_adapter_on_small_trees() -> None:
    ...
    for num_nodes, edges in cases:
        ...
        reference = competitor.layout_with_variant(graph, variant_params={"algorithm": "cluster"})
...
def test_layout_config_algorithm_d3_cluster_works() -> None:
    """Exercise public engine dispatch for ``algorithm='d3_cluster'``.
    ...
    positions = layout(graph, LayoutConfig(algorithm="d3_cluster"))
    assert positions.shape == (3, 2)
    assert torch.isfinite(positions).all()


def test_d3_cluster_radial_variant_runs() -> None:
    """Exercise the named radial cluster variant.
    ...
    positions = layout_d3_cluster_radial_pipeline(edge_index, 4)
    assert positions.shape == (4, 2)
    assert torch.isfinite(positions).all()


def test_d3_cluster_production_pipeline_has_no_runtime_delegation() -> None:
    """Guard production d3 cluster code against Node/reference delegation.
    ...
    source = "\n".join(
        [
            ...
            (root / "dagua" / "layout" / "ops" / "pipelines" / "d3_cluster.py").read_text(),
        ]
    )
