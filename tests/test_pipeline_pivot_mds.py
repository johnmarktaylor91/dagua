"""Exact-fidelity tests for the composable Pivot-MDS pipeline."""
...
def _assert_exact_match(classic: torch.Tensor, pipeline: torch.Tensor) -> None:
    ...
    classic : torch.Tensor
        Reference output from classic Pivot MDS.
...
def _run_pipeline_direct(
    ...
    *,
    ...
    """Execute ``build_pivot_mds_pipeline`` directly on a fresh solve state.
...
class TestPivotMDSPipelineFidelity:
    """Bit-exact regression coverage for the Pivot-MDS pipeline."""

    @pytest.mark.parametrize(
        ("num_nodes", "seed", "n_pivots"),
        [(0, 42, 5), (1, 42, 5), (2, 42, 5), (5, 42, 5), (5, 99, 3), (20, 42, 8)],
    )
    def test_layout_pivot_mds_pipeline_matches_classic_for_requested_sizes(
        self,
        num_nodes: int,
        seed: int,
        n_pivots: int,
    ) -> None:
        """The adapter should match classic Pivot MDS exactly."""
    ...
    def test_layout_pivot_mds_pipeline_matches_classic_on_disconnected_graph(self) -> None:
        ...
        classic = layout_pivot_mds(edge_index=edge_index, num_nodes=9, n_pivots=6, seed=99)
        pipeline = layout_pivot_mds_pipeline(
            edge_index=edge_index,
            num_nodes=9,
            n_pivots=6,
    ...
    def test_build_pivot_mds_pipeline_matches_classic_on_complete_graph(self) -> None:
        """The raw pipeline object should match classic Pivot MDS on a dense graph."""
        edge_index = _complete_edge_index(6)

        classic = layout_pivot_mds(edge_index=edge_index, num_nodes=6, n_pivots=5, seed=7)
        pipeline = _run_pipeline_direct(edge_index=edge_index, num_nodes=6, n_pivots=5, seed=7)

        _assert_exact_match(classic, pipeline)

    def test_layout_pivot_mds_pipeline_matches_classic_with_node_sizes(self) -> None:
