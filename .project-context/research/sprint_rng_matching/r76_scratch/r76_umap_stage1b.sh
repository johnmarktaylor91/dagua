#!/usr/bin/env bash
# r76 umap stage-1b: bench post-revert worktree code, rescore vs honest seed-100-199 refs.
set -uo pipefail
...
nice -n 10 python3 scripts/run_benchmark.py --variants \
  ...
  --seeds 100 --seed-start 100 --workers 5 --timeout 2400 --watchdog-timeout 3600 \
  --output-dir $MAIN/eval_output/benchmark_100seed_r76_umap_fix2
echo "BENCH_RC=$?"
...
nice -n 10 python3 scripts/definitive_fidelity_analysis.py --mode full \
  ...
  --data-dir eval_output/benchmark_100seed_r76_umap_fix2 \
  --combos-file /tmp/r76_umap_combos.txt --workers 6 \
  --output eval_output/fidelity_definitive/r76_umap_stage1b.jsonl
echo "RESCORE_RC=$?"

python3 - <<'PY'
import json
rows=[json.loads(l) for l in open('eval_output/fidelity_definitive/r76_umap_stage1b.jsonl')]
last={}
...
print("=== the 7 previously-divergent (honest refs, full power) ===")
for r in sorted(rows,key=lambda r:r['combo_id']):
    if r['combo_id'] in div7:
        print(f"  {r['combo_id']:55s} n={r.get('n')} ident={r.get('quality_identical_raw')} equiv={r.get('quality_equivalent_raw')} W_D={r.get('mean_W_D')} W_R={r.get('mean_W_R')}")
print("=== regression sample (must be identical) ===")
for r in sorted(rows,key=lambda r:r['combo_id']):
    if r['combo_id'].split('::')[0] in ('citation_dag_300','clustered_longlabel_handoffs'):
        print(f"  {r['combo_id']:55s} n={r.get('n')} ident={r.get('quality_identical_raw')} equiv={r.get('quality_equivalent_raw')}")
PY
echo "R76_UMAP_STAGE1B_COMPLETE"
