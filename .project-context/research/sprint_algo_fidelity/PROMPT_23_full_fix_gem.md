...
Round 23 EXHAUSTIVE SWEEP for **gem** family (`classic_gem` vs `ogdf_gem`).

The user explicitly directed: **"plz fix ALL issues you found"** -- not just top 3.

## Round 22 status: residual

Round 22 was RESIDUAL (no commit). Read the existing ROUND_22_RESIDUAL_gem.md (if present) to understand what was tried + reverted. Round 23 should retry the top items (potentially with adjusted scope/scaffolding) AND apply remaining items #4+.

## SPEC

Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_gem.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_gem.md` reports for context.
...
1. Read ROUND_21_DIFF_gem.md fully + any ROUND_22_*_gem.md.
2. Multi-seed baseline (use the bounded 5-graph subset):
   ...
   python scripts/algo_fidelity_live_compare.py classic_gem ogdf_gem \
       ...
       --output-dir eval_output/algo_fidelity/round_23/gem/baseline
   ```
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 gem -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "gem"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_gem ogdf_gem \
       --seeds 3 \
       --graphs linear_3layer_mlp,parallel_multiedge_bundle,nested_shallow_enc_dec,tl_mlp_3layer,mixed_width_labels \
       --output-dir eval_output/algo_fidelity/round_23/gem/post_fix
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/gem/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_gem "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/gem/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*gem*.md`
- `tests/test_layout/test_*gem*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "gem"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/gem/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
gem family only. May commit MULTIPLE times. Stage specific files only.
