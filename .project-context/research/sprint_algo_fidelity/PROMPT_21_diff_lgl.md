...
Round 21 ADVERSARIAL DIFF for **lgl** family (dagua `classic_lgl` vs reference `igraph_lgl`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/lgl.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/lgl.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs igraph_lgl.

**Reference side (READ ALL):**
- Reference path hint: `igraph/src/layout/large_graph_layout.c`
...
- `eval_output/fidelity_report/report.md` for the current verdict on lgl.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_lgl.md`
...
<verification_loop>File ROUND_21_DIFF_lgl.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
