...
Round 21 ADVERSARIAL DIFF for **sugiyama** family (dagua `classic_sugiyama` vs reference `igraph_sugiyama`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/sugiyama.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/sugiyama.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs igraph_sugiyama.

**Reference side (READ ALL):**
- Reference path hint: `igraph/src/layout/sugiyama`
...
- `eval_output/fidelity_report/report.md` for the current verdict on sugiyama.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_sugiyama.md`
...
<verification_loop>File ROUND_21_DIFF_sugiyama.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
