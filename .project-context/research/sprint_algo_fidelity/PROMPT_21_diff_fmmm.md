...
Round 21 ADVERSARIAL DIFF for **fmmm** family (dagua `classic_fmmm` vs reference `ogdf_fmmm`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/fmmm.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/fmmm.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs ogdf_fmmm.

**Reference side (READ ALL):**
- Reference path hint: `ogdf/src/ogdf/energybased/FMMM`
...
- `eval_output/fidelity_report/report.md` for the current verdict on fmmm.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_fmmm.md`
...
<verification_loop>File ROUND_21_DIFF_fmmm.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
