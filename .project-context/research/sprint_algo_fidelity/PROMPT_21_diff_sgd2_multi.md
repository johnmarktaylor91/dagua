...
Round 21 ADVERSARIAL DIFF for **sgd2_multi** family (dagua `classic_sgd2_multi` vs reference `sgd2_multi_ref`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/sgd2_multi.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/sgd2_multi.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs sgd2_multi_ref.

**Reference side (READ ALL):**
- Reference path hint: `site-packages/sgd2`
...
- `eval_output/fidelity_report/report.md` for the current verdict on sgd2_multi.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_sgd2_multi.md`
...
<verification_loop>File ROUND_21_DIFF_sgd2_multi.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
