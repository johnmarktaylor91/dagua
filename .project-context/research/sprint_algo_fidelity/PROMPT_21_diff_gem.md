...
Round 21 ADVERSARIAL DIFF for **gem** family (dagua `classic_gem` vs reference `ogdf_gem`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/gem.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/gem.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs ogdf_gem.

**Reference side (READ ALL):**
- Reference path hint: `ogdf/src/ogdf/energybased/GEM`
...
- `eval_output/fidelity_report/report.md` for the current verdict on gem.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_gem.md`
...
<verification_loop>File ROUND_21_DIFF_gem.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
