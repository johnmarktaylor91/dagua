...
Round 23 EXHAUSTIVE SWEEP for **spectral** family (`classic_spectral` vs `nx_spectral`).
...
Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_spectral.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_spectral.md` reports for context.
...
1. Read ROUND_21_DIFF_spectral.md fully + any ROUND_22_*_spectral.md.
...
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 spectral -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "spectral"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_spectral nx_spectral \
       --seeds 3 \
   ...
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/spectral/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_spectral "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/spectral/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*spectral*.md`
- `tests/test_layout/test_*spectral*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "spectral"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/spectral/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
spectral family only. May commit MULTIPLE times. Stage specific files only.
