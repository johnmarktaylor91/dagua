...
Round 21 ADVERSARIAL DIFF for **classical_mds** family (dagua `classic_classical_mds` vs reference `igraph_mds`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/classical_mds.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/classical_mds.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs igraph_mds.

**Reference side (READ ALL):**
- Reference path hint: `igraph/src/layout/mds.c`
...
- `eval_output/fidelity_report/report.md` for the current verdict on classical_mds.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_classical_mds.md`
...
<verification_loop>File ROUND_21_DIFF_classical_mds.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
