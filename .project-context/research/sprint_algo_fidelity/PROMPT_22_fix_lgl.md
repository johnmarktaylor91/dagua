...
Round 22 ADVERSARIAL FIX for **lgl** family (`classic_lgl` vs `igraph_lgl`).
...
1. Read `ROUND_21_DIFF_lgl.md` end-to-end.
...
4. Run pytest tests/test_layout/ -x --tb=short -q -k "lgl" (or
...
7. If COMMITTED: `feat(fidelity): round 22 lgl -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_lgl.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/lgl/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for lgl (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/lgl/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*lgl*.md`
- `tests/test_layout/test_*lgl*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own lgl)
...
<scope_constraints>lgl family files only. NO other family code.</scope_constraints>
