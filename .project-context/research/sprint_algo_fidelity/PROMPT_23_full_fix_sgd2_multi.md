...
Round 23 EXHAUSTIVE SWEEP for **sgd2_multi** family (`classic_sgd2_multi` vs `sgd2_multi_ref`).

The user explicitly directed: **"plz fix ALL issues you found"** -- not just top 3.

## Round 22 status: residual

Round 22 was RESIDUAL (no commit). Read the existing ROUND_22_RESIDUAL_sgd2_multi.md (if present) to understand what was tried + reverted. Round 23 should retry the top items (potentially with adjusted scope/scaffolding) AND apply remaining items #4+.

## SPEC

Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_sgd2_multi.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_sgd2_multi.md` reports for context.
...
1. Read ROUND_21_DIFF_sgd2_multi.md fully + any ROUND_22_*_sgd2_multi.md.
2. Multi-seed baseline (use the bounded 5-graph subset):
   ```
   python scripts/algo_fidelity_live_compare.py classic_sgd2_multi sgd2_multi_ref \
       --seeds 3 \
       --graphs linear_3layer_mlp,parallel_multiedge_bundle,nested_shallow_enc_dec,tl_mlp_3layer,mixed_width_labels \
       --output-dir eval_output/algo_fidelity/round_23/sgd2_multi/baseline
   ```
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 sgd2_multi -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "sgd2_multi"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_sgd2_multi sgd2_multi_ref \
       --seeds 3 \
       --graphs linear_3layer_mlp,parallel_multiedge_bundle,nested_shallow_enc_dec,tl_mlp_3layer,mixed_width_labels \
       --output-dir eval_output/algo_fidelity/round_23/sgd2_multi/post_fix
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/sgd2_multi/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_sgd2_multi "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/sgd2_multi/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*sgd2_multi*.md`
- `tests/test_layout/test_*sgd2_multi*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "sgd2_multi"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/sgd2_multi/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
sgd2_multi family only. May commit MULTIPLE times. Stage specific files only.
