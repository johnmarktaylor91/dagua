...
Round 21 ADVERSARIAL DIFF for **kk** family (dagua `classic_kk` vs reference `nx_kamada_kawai`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/kk.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/kk.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs nx_kamada_kawai.
...
- `eval_output/fidelity_report/report.md` for the current verdict on kk.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_kk.md`
...
<verification_loop>File ROUND_21_DIFF_kk.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
