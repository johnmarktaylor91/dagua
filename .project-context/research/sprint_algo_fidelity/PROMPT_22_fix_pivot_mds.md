...
Round 22 ADVERSARIAL FIX for **pivot_mds** family (`classic_pivot_mds` vs `ogdf_pivot_mds`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_pivot_mds.md`
...
1. Read `ROUND_21_DIFF_pivot_mds.md` end-to-end.
2. Multi-seed baseline (3 seeds, 5 small graphs):
   ...
   python scripts/algo_fidelity_live_compare.py classic_pivot_mds ogdf_pivot_mds \
       ...
       --output-dir eval_output/algo_fidelity/round_22/pivot_mds/baseline
   ```
3. Apply the top 3 levers from the spec as a bundle. Be precise --
   cite line:line refs from the diff doc.
4. Run pytest tests/test_layout/ -x --tb=short -q -k "pivot_mds" (or
...
7. If COMMITTED: `feat(fidelity): round 22 pivot_mds -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_pivot_mds.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/pivot_mds/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for pivot_mds (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/pivot_mds/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*pivot_mds*.md`
- `tests/test_layout/test_*pivot_mds*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own pivot_mds)
...
<scope_constraints>pivot_mds family files only. NO other family code.</scope_constraints>
