...
Round 22 ADVERSARIAL FIX for **maxent_stress** family (`classic_maxent_stress` vs `ogdf_stress`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_maxent_stress.md`
...
1. Read `ROUND_21_DIFF_maxent_stress.md` end-to-end.
2. Multi-seed baseline (3 seeds, 5 small graphs):
   ...
   python scripts/algo_fidelity_live_compare.py classic_maxent_stress ogdf_stress \
       ...
       --output-dir eval_output/algo_fidelity/round_22/maxent_stress/baseline
   ```
3. Apply the top 3 levers from the spec as a bundle. Be precise --
   cite line:line refs from the diff doc.
4. Run pytest tests/test_layout/ -x --tb=short -q -k "maxent_stress" (or
...
7. If COMMITTED: `feat(fidelity): round 22 maxent_stress -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_maxent_stress.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/maxent_stress/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for maxent_stress (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/maxent_stress/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*maxent_stress*.md`
- `tests/test_layout/test_*maxent_stress*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own maxent_stress)
...
<scope_constraints>maxent_stress family files only. NO other family code.</scope_constraints>
