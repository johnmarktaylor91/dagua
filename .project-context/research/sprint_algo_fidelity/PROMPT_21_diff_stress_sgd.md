...
Round 21 ADVERSARIAL DIFF for **stress_sgd** family (dagua `classic_stress_sgd` vs reference `sgd2`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/stress_sgd.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/stress_sgd.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs sgd2.
...
- `eval_output/fidelity_report/report.md` for the current verdict on stress_sgd.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_stress_sgd.md`
...
<verification_loop>File ROUND_21_DIFF_stress_sgd.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
