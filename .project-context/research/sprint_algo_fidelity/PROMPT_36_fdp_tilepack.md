<task>
R36 BIT-EXACT graphviz port. Sub-task: **fdp / tile packing for components**.
...
Port the tile packing for components component of graphviz fdp from C to Python/PyTorch,
...
2. Implement the tile packing for components logic in Python/PyTorch under `fidelity_mode`.
...
Focus: after laying out connected components separately, pack them via tile-based bin-packing into a single bounding box. Affects component-level placement.
...
- Commit format: `feat(layout): round 36 fdp_tilepack -- <terse>`. Multi-commit OK.

## Output

`eval_output/algo_fidelity/round_36/fdp_tilepack/SUMMARY.md` with:
- Source files read
