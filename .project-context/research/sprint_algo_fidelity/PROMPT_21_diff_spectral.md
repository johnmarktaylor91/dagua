...
Round 21 ADVERSARIAL DIFF for **spectral** family (dagua `classic_spectral` vs reference `nx_spectral`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/spectral.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/spectral.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs nx_spectral.
...
- `eval_output/fidelity_report/report.md` for the current verdict on spectral.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_spectral.md`
...
<verification_loop>File ROUND_21_DIFF_spectral.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
