...
Round 21 ADVERSARIAL DIFF for **pivot_mds** family (dagua `classic_pivot_mds` vs reference `ogdf_pivot_mds`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/pivot_mds.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/pivot_mds.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs ogdf_pivot_mds.

**Reference side (READ ALL):**
- Reference path hint: `ogdf/src/ogdf/energybased/PivotMDS`
...
- `eval_output/fidelity_report/report.md` for the current verdict on pivot_mds.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_pivot_mds.md`
...
<verification_loop>File ROUND_21_DIFF_pivot_mds.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
