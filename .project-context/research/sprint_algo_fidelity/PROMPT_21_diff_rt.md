...
Round 21 ADVERSARIAL DIFF for **rt** family (dagua `classic_rt` vs reference `igraph_rt`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/rt.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/rt.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs igraph_rt.

**Reference side (READ ALL):**
- Reference path hint: `igraph/src/layout/reingold_tilford.c`
...
- `eval_output/fidelity_report/report.md` for the current verdict on rt.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_rt.md`
...
<verification_loop>File ROUND_21_DIFF_rt.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
