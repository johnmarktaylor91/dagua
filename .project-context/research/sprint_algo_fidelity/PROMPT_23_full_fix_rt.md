...
Round 23 EXHAUSTIVE SWEEP for **rt** family (`classic_rt` vs `igraph_rt`).
...
Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_rt.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_rt.md` reports for context.
...
1. Read ROUND_21_DIFF_rt.md fully + any ROUND_22_*_rt.md.
...
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 rt -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "rt"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_rt igraph_rt \
       --seeds 3 \
   ...
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/rt/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_rt "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/rt/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*rt*.md`
- `tests/test_layout/test_*rt*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "rt"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/rt/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
rt family only. May commit MULTIPLE times. Stage specific files only.
