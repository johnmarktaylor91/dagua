...
Round 22 ADVERSARIAL FIX for **sugiyama** family (`classic_sugiyama` vs `igraph_sugiyama`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_sugiyama.md`
...
1. Read `ROUND_21_DIFF_sugiyama.md` end-to-end.
...
4. Run pytest tests/test_layout/ -x --tb=short -q -k "sugiyama" (or
...
7. If COMMITTED: `feat(fidelity): round 22 sugiyama -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_sugiyama.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/sugiyama/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for sugiyama (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/sugiyama/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*sugiyama*.md`
- `tests/test_layout/test_*sugiyama*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own sugiyama)
...
<scope_constraints>sugiyama family files only. NO other family code.</scope_constraints>
