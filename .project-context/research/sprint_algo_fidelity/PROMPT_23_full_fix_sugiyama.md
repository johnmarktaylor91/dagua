...
Round 23 EXHAUSTIVE SWEEP for **sugiyama** family (`classic_sugiyama` vs `igraph_sugiyama`).
...
Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_sugiyama.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_sugiyama.md` reports for context.
...
1. Read ROUND_21_DIFF_sugiyama.md fully + any ROUND_22_*_sugiyama.md.
...
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 sugiyama -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "sugiyama"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_sugiyama igraph_sugiyama \
       --seeds 3 \
   ...
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/sugiyama/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_sugiyama "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/sugiyama/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*sugiyama*.md`
- `tests/test_layout/test_*sugiyama*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "sugiyama"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/sugiyama/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
sugiyama family only. May commit MULTIPLE times. Stage specific files only.
