...
Round 23 EXHAUSTIVE SWEEP for **stress_maj** family (`classic_stress_maj` vs `ogdf_stress`).
...
Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_stress_maj.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_stress_maj.md` reports for context.
...
1. Read ROUND_21_DIFF_stress_maj.md fully + any ROUND_22_*_stress_maj.md.
2. Multi-seed baseline (use the bounded 5-graph subset):
   ```
   python scripts/algo_fidelity_live_compare.py classic_stress_maj ogdf_stress \
       --seeds 3 \
       --graphs linear_3layer_mlp,parallel_multiedge_bundle,nested_shallow_enc_dec,tl_mlp_3layer,mixed_width_labels \
       --output-dir eval_output/algo_fidelity/round_23/stress_maj/baseline
   ```
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 stress_maj -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "stress_maj"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_stress_maj ogdf_stress \
       --seeds 3 \
       --graphs linear_3layer_mlp,parallel_multiedge_bundle,nested_shallow_enc_dec,tl_mlp_3layer,mixed_width_labels \
       --output-dir eval_output/algo_fidelity/round_23/stress_maj/post_fix
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/stress_maj/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_stress_maj "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/stress_maj/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*stress_maj*.md`
- `tests/test_layout/test_*stress_maj*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "stress_maj"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/stress_maj/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
stress_maj family only. May commit MULTIPLE times. Stage specific files only.
