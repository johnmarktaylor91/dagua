...
Round 22 ADVERSARIAL FIX for **kk** family (`classic_kk` vs `nx_kamada_kawai`).
...
1. Read `ROUND_21_DIFF_kk.md` end-to-end.
...
4. Run pytest tests/test_layout/ -x --tb=short -q -k "kk" (or
...
7. If COMMITTED: `feat(fidelity): round 22 kk -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_kk.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/kk/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for kk (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/kk/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*kk*.md`
- `tests/test_layout/test_*kk*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own kk)
...
<scope_constraints>kk family files only. NO other family code.</scope_constraints>
