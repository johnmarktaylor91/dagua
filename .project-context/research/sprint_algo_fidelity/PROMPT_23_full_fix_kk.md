...
Round 23 EXHAUSTIVE SWEEP for **kk** family (`classic_kk` vs `nx_kamada_kawai`).
...
Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_kk.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_kk.md` reports for context.
...
1. Read ROUND_21_DIFF_kk.md fully + any ROUND_22_*_kk.md.
2. Multi-seed baseline (use the bounded 5-graph subset):
   ...
   python scripts/algo_fidelity_live_compare.py classic_kk nx_kamada_kawai \
       ...
       --output-dir eval_output/algo_fidelity/round_23/kk/baseline
   ```
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 kk -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "kk"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_kk nx_kamada_kawai \
       --seeds 3 \
       --graphs linear_3layer_mlp,parallel_multiedge_bundle,nested_shallow_enc_dec,tl_mlp_3layer,mixed_width_labels \
       --output-dir eval_output/algo_fidelity/round_23/kk/post_fix
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/kk/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_kk "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/kk/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*kk*.md`
- `tests/test_layout/test_*kk*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "kk"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/kk/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
kk family only. May commit MULTIPLE times. Stage specific files only.
