...
Round 23 EXHAUSTIVE SWEEP for **lgl** family (`classic_lgl` vs `igraph_lgl`).
...
Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_lgl.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_lgl.md` reports for context.
...
1. Read ROUND_21_DIFF_lgl.md fully + any ROUND_22_*_lgl.md.
...
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 lgl -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "lgl"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_lgl igraph_lgl \
       --seeds 3 \
   ...
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/lgl/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_lgl "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/lgl/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*lgl*.md`
- `tests/test_layout/test_*lgl*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "lgl"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/lgl/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
lgl family only. May commit MULTIPLE times. Stage specific files only.
