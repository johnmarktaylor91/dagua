...
Round 22 ADVERSARIAL FIX for **stress_sgd** family (`classic_stress_sgd` vs `sgd2`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_stress_sgd.md`
...
1. Read `ROUND_21_DIFF_stress_sgd.md` end-to-end.
...
4. Run pytest tests/test_layout/ -x --tb=short -q -k "stress_sgd" (or
...
7. If COMMITTED: `feat(fidelity): round 22 stress_sgd -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_stress_sgd.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/stress_sgd/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for stress_sgd (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/stress_sgd/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*stress_sgd*.md`
- `tests/test_layout/test_*stress_sgd*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own stress_sgd)
...
<scope_constraints>stress_sgd family files only. NO other family code.</scope_constraints>
