<task>
R36 BIT-EXACT graphviz port. Sub-task: **sfdp / matrix-based coarsening**.
...
Port the matrix-based coarsening component of graphviz sfdp from C to Python/PyTorch,
...
- /home/jtaylor/projects/_references/graphviz/lib/sfdpgen/Multilevel.c (matrix-based supervariable clustering)
...
2. Implement the matrix-based coarsening logic in Python/PyTorch under `fidelity_mode`.
...
Focus: graphviz sfdp coarsening builds the coarse graph via matrix multiplication on a supervariable clustering; dagua uses heavy-edge matching. Port the matrix-based path.
...
- Commit format: `feat(layout): round 36 sfdp_coarsening -- <terse>`. Multi-commit OK.

## Output

`eval_output/algo_fidelity/round_36/sfdp_coarsening/SUMMARY.md` with:
- Source files read
