...
Round 21 ADVERSARIAL DIFF for **stress_maj** family (dagua `classic_stress_maj` vs reference `ogdf_stress`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/stress_maj.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/stress_maj.py` (the pipeline wiring).
...
- Reference path hint: `ogdf/src/ogdf/energybased/StressMinimization`
...
- `eval_output/fidelity_report/report.md` for the current verdict on stress_maj.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_stress_maj.md`
...
<verification_loop>File ROUND_21_DIFF_stress_maj.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
