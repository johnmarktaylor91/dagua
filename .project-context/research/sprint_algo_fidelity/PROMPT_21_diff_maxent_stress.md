...
Round 21 ADVERSARIAL DIFF for **maxent_stress** family (dagua `classic_maxent_stress` vs reference `ogdf_stress`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/maxent_stress.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/maxent_stress.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs ogdf_stress.

**Reference side (READ ALL):**
- Reference path hint: `ogdf/src/ogdf/energybased`
...
- `eval_output/fidelity_report/report.md` for the current verdict on maxent_stress.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_maxent_stress.md`
...
<verification_loop>File ROUND_21_DIFF_maxent_stress.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
