...
Round 23 EXHAUSTIVE SWEEP for **fa2** family (`classic_fa2` vs `fa2_ref`).
...
Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_fa2.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_fa2.md` reports for context.
...
1. Read ROUND_21_DIFF_fa2.md fully + any ROUND_22_*_fa2.md.
...
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 fa2 -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "fa2"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_fa2 fa2_ref \
       --seeds 3 \
   ...
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/fa2/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_fa2 "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/fa2/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*fa2*.md`
- `tests/test_layout/test_*fa2*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "fa2"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/fa2/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
fa2 family only. May commit MULTIPLE times. Stage specific files only.
