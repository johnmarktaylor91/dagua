<task>
R36 BIT-EXACT graphviz port. Sub-task: **fdp / derived-graph recursion + cluster expansion**.
...
Port the derived-graph recursion + cluster expansion component of graphviz fdp from C to Python/PyTorch,
...
- /home/jtaylor/projects/_references/graphviz/lib/fdpgen/comp.c (component decomposition + cluster expansion)
- /home/jtaylor/projects/_references/graphviz/lib/fdpgen/layout.c (overall fdp recursion)

## Dagua target files

- dagua/layout/ops/pipelines/fmmm.py (fdp dispatches through fmmm; add graphviz_fdp_fidelity)

## Implementation

1. Read the reference C files end-to-end.
2. Implement the derived-graph recursion + cluster expansion logic in Python/PyTorch under `fidelity_mode`.
...
Focus: graphviz fdp recursively lays out cluster-derived sub-graphs, then composes. Different from OGDF FMMM coarsening.
...
- Commit format: `feat(layout): round 36 fdp_recursion -- <terse>`. Multi-commit OK.

## Output

`eval_output/algo_fidelity/round_36/fdp_recursion/SUMMARY.md` with:
- Source files read
