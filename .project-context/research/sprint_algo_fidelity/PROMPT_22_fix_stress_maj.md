...
Round 22 ADVERSARIAL FIX for **stress_maj** family (`classic_stress_maj` vs `ogdf_stress`).
...
1. Read `ROUND_21_DIFF_stress_maj.md` end-to-end.
2. Multi-seed baseline (3 seeds, 5 small graphs):
   ...
   python scripts/algo_fidelity_live_compare.py classic_stress_maj ogdf_stress \
       ...
       --output-dir eval_output/algo_fidelity/round_22/stress_maj/baseline
   ```
3. Apply the top 3 levers from the spec as a bundle. Be precise --
   cite line:line refs from the diff doc.
4. Run pytest tests/test_layout/ -x --tb=short -q -k "stress_maj" (or
...
7. If COMMITTED: `feat(fidelity): round 22 stress_maj -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_stress_maj.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/stress_maj/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for stress_maj (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/stress_maj/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*stress_maj*.md`
- `tests/test_layout/test_*stress_maj*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own stress_maj)
...
<scope_constraints>stress_maj family files only. NO other family code.</scope_constraints>
