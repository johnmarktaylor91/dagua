...
Round 21 ADVERSARIAL DIFF for **fa2** family (dagua `classic_fa2` vs reference `fa2_ref`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/fa2.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/fa2.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs fa2_ref.

**Reference side (READ ALL):**
- Reference path hint: `site-packages/fa2_modified`
...
- `eval_output/fidelity_report/report.md` for the current verdict on fa2.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_fa2.md`
...
<verification_loop>File ROUND_21_DIFF_fa2.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
