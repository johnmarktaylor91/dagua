...
Round 22 ADVERSARIAL FIX for **gem** family (`classic_gem` vs `ogdf_gem`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_gem.md`
...
1. Read `ROUND_21_DIFF_gem.md` end-to-end.
2. Multi-seed baseline (3 seeds, 5 small graphs):
   ...
   python scripts/algo_fidelity_live_compare.py classic_gem ogdf_gem \
       ...
       --output-dir eval_output/algo_fidelity/round_22/gem/baseline
   ```
3. Apply the top 3 levers from the spec as a bundle. Be precise --
   cite line:line refs from the diff doc.
4. Run pytest tests/test_layout/ -x --tb=short -q -k "gem" (or
...
7. If COMMITTED: `feat(fidelity): round 22 gem -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_gem.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/gem/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for gem (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/gem/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*gem*.md`
- `tests/test_layout/test_*gem*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own gem)
...
<scope_constraints>gem family files only. NO other family code.</scope_constraints>
