...
Round 22 ADVERSARIAL FIX for **spectral** family (`classic_spectral` vs `nx_spectral`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_spectral.md`
...
1. Read `ROUND_21_DIFF_spectral.md` end-to-end.
2. Multi-seed baseline (3 seeds, 5 small graphs):
   ...
   python scripts/algo_fidelity_live_compare.py classic_spectral nx_spectral \
       ...
       --output-dir eval_output/algo_fidelity/round_22/spectral/baseline
   ```
3. Apply the top 3 levers from the spec as a bundle. Be precise --
   cite line:line refs from the diff doc.
4. Run pytest tests/test_layout/ -x --tb=short -q -k "spectral" (or
...
7. If COMMITTED: `feat(fidelity): round 22 spectral -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_spectral.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/spectral/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for spectral (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/spectral/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*spectral*.md`
- `tests/test_layout/test_*spectral*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own spectral)
...
<scope_constraints>spectral family files only. NO other family code.</scope_constraints>
