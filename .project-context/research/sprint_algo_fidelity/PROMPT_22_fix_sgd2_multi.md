...
Round 22 ADVERSARIAL FIX for **sgd2_multi** family (`classic_sgd2_multi` vs `sgd2_multi_ref`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_sgd2_multi.md`
...
1. Read `ROUND_21_DIFF_sgd2_multi.md` end-to-end.
2. Multi-seed baseline (3 seeds, 5 small graphs):
   ...
   python scripts/algo_fidelity_live_compare.py classic_sgd2_multi sgd2_multi_ref \
       ...
       --output-dir eval_output/algo_fidelity/round_22/sgd2_multi/baseline
   ```
3. Apply the top 3 levers from the spec as a bundle. Be precise --
   cite line:line refs from the diff doc.
4. Run pytest tests/test_layout/ -x --tb=short -q -k "sgd2_multi" (or
...
7. If COMMITTED: `feat(fidelity): round 22 sgd2_multi -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_sgd2_multi.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/sgd2_multi/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for sgd2_multi (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/sgd2_multi/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*sgd2_multi*.md`
- `tests/test_layout/test_*sgd2_multi*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own sgd2_multi)
...
<scope_constraints>sgd2_multi family files only. NO other family code.</scope_constraints>
