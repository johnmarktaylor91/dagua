...
Round 21 ADVERSARIAL DIFF for **umap** family (dagua `classic_umap` vs reference `umap_graph`).
...
**Dagua side (READ ALL):**
- Locate `dagua/layout/ops/umap.py` or related ops files for this engine.
- Locate `dagua/layout/ops/pipelines/umap.py` (the pipeline wiring).
- `dagua/eval/variants.py` for variant configs.
- `dagua/eval/competitors/` for the adapter that runs umap_graph.

**Reference side (READ ALL):**
- Reference path hint: `site-packages/umap`
...
- `eval_output/fidelity_report/report.md` for the current verdict on umap.
...
Produce ONE document: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_umap.md`
...
<verification_loop>File ROUND_21_DIFF_umap.md exists and is exhaustive (>10KB) with line:line refs throughout.</verification_loop>
