...
Round 23 EXHAUSTIVE SWEEP for **stress_sgd** family (`classic_stress_sgd` vs `sgd2`).
...
Primary: `.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_stress_sgd.md` (full ranked fix list).
Secondary: existing `ROUND_22_*_stress_sgd.md` reports for context.
...
1. Read ROUND_21_DIFF_stress_sgd.md fully + any ROUND_22_*_stress_sgd.md.
2. Multi-seed baseline (use the bounded 5-graph subset):
   ...
   python scripts/algo_fidelity_live_compare.py classic_stress_sgd sgd2 \
       ...
       --output-dir eval_output/algo_fidelity/round_23/stress_sgd/baseline
   ```
3. Apply remaining ranked items. Each as its own micro-commit OR bundle
   related items. Commit messages: `feat(fidelity): round 23 stress_sgd -- <short>`.
4. Run `pytest tests/test_layout/ -x --tb=short -q -k "stress_sgd"` after each commit.
5. Final measure:
   ```
   python scripts/algo_fidelity_live_compare.py classic_stress_sgd sgd2 \
       --seeds 3 \
       --graphs linear_3layer_mlp,parallel_multiedge_bundle,nested_shallow_enc_dec,tl_mlp_3layer,mixed_width_labels \
       --output-dir eval_output/algo_fidelity/round_23/stress_sgd/post_fix
   ```
6. Per-round summary: `eval_output/algo_fidelity/round_23/stress_sgd/SUMMARY.md`
...
**Allowed**:
- Family-specific ops/pipeline files (per ROUND_21_DIFF_stress_sgd "Files Read" section)
...
- `eval_output/algo_fidelity/round_23/stress_sgd/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_23_*stress_sgd*.md`
- `tests/test_layout/test_*stress_sgd*.py` for regressions
...
After each commit:
- `pytest tests/test_layout/ -x --tb=short -q -k "stress_sgd"` passes
- `git diff --stat HEAD~1 HEAD` shows ONLY your family's files

Final state:
- `eval_output/algo_fidelity/round_23/stress_sgd/SUMMARY.md` lists every item attempted, with status (commit hash | reverted | skipped + reason)
</task>

<scope_constraints>
stress_sgd family only. May commit MULTIPLE times. Stage specific files only.
