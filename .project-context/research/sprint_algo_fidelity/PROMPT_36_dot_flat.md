<task>
R36 BIT-EXACT graphviz port. Sub-task: **dot / flat/self/multi-edge handling**.
...
Port the flat/self/multi-edge handling component of graphviz dot from C to Python/PyTorch,
...
2. Implement the flat/self/multi-edge handling logic in Python/PyTorch under `fidelity_mode`.
...
Focus: same-rank edges (flat), self-loops, multi-edges between same node pair. These get special routing/positioning treatment in dot.
...
- Commit format: `feat(layout): round 36 dot_flat -- <terse>`. Multi-commit OK.

## Output

`eval_output/algo_fidelity/round_36/dot_flat/SUMMARY.md` with:
- Source files read
