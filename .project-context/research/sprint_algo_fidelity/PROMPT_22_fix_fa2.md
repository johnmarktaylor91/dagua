...
Round 22 ADVERSARIAL FIX for **fa2** family (`classic_fa2` vs `fa2_ref`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_fa2.md`
...
1. Read `ROUND_21_DIFF_fa2.md` end-to-end.
2. Multi-seed baseline (3 seeds, 5 small graphs):
   ...
   python scripts/algo_fidelity_live_compare.py classic_fa2 fa2_ref \
       ...
       --output-dir eval_output/algo_fidelity/round_22/fa2/baseline
   ```
3. Apply the top 3 levers from the spec as a bundle. Be precise --
   cite line:line refs from the diff doc.
4. Run pytest tests/test_layout/ -x --tb=short -q -k "fa2" (or
...
7. If COMMITTED: `feat(fidelity): round 22 fa2 -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_fa2.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/fa2/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for fa2 (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/fa2/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*fa2*.md`
- `tests/test_layout/test_*fa2*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own fa2)
...
<scope_constraints>fa2 family files only. NO other family code.</scope_constraints>
