<task>
R36 BIT-EXACT graphviz port. Sub-task: **neato / PRISM/VPSC overlap removal**.
...
Port the PRISM/VPSC overlap removal component of graphviz neato from C to Python/PyTorch,
...
- /home/jtaylor/projects/_references/graphviz/lib/neatogen/quad_prog_solver.c (VPSC quadratic programming)

## Dagua target files

- dagua/layout/ops/pipelines/neato.py (add overlap_removal=True option)

## Implementation

1. Read the reference C files end-to-end.
2. Implement the PRISM/VPSC overlap removal logic in Python/PyTorch under `fidelity_mode`.
...
Focus: PRISM algorithm + VPSC (Variable Placement with Separation Constraints) quadratic programming. After stress layout, remove node overlaps while preserving structure.
...
- Commit format: `feat(layout): round 36 neato_overlap -- <terse>`. Multi-commit OK.

## Output

`eval_output/algo_fidelity/round_36/neato_overlap/SUMMARY.md` with:
- Source files read
