...
Round 22 ADVERSARIAL FIX for **rt** family (`classic_rt` vs `igraph_rt`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_rt.md`
...
1. Read `ROUND_21_DIFF_rt.md` end-to-end.
...
4. Run pytest tests/test_layout/ -x --tb=short -q -k "rt" (or
...
7. If COMMITTED: `feat(fidelity): round 22 rt -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_rt.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/rt/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for rt (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/rt/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*rt*.md`
- `tests/test_layout/test_*rt*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own rt)
...
<scope_constraints>rt family files only. NO other family code.</scope_constraints>
