<task>
R36 BIT-EXACT graphviz port. Sub-task: **dot / x-position network simplex**.
...
Port the x-position network simplex component of graphviz dot from C to Python/PyTorch,
...
- /home/jtaylor/projects/_references/graphviz/lib/dotgen/position.c (different network simplex problem; solves x-coordinates given ranks + ordering)
...
2. Implement the x-position network simplex logic in Python/PyTorch under `fidelity_mode`.
...
Focus: given ranks + within-rank ordering from mincross, solve x-positions via network simplex. Takes ranks + orderings + node widths, returns x-coords per node. Different graph than rank assignment's network simplex.
...
- Commit format: `feat(layout): round 36 dot_position -- <terse>`. Multi-commit OK.

## Output

`eval_output/algo_fidelity/round_36/dot_position/SUMMARY.md` with:
- Source files read
