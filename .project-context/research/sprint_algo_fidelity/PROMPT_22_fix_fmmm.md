...
Round 22 ADVERSARIAL FIX for **fmmm** family (`classic_fmmm` vs `ogdf_fmmm`).

## SPEC

Your spec is the diff document at:
`.project-context/research/sprint_algo_fidelity/ROUND_21_DIFF_fmmm.md`
...
1. Read `ROUND_21_DIFF_fmmm.md` end-to-end.
2. Multi-seed baseline (3 seeds, 5 small graphs):
   ...
   python scripts/algo_fidelity_live_compare.py classic_fmmm ogdf_fmmm \
       ...
       --output-dir eval_output/algo_fidelity/round_22/fmmm/baseline
   ```
3. Apply the top 3 levers from the spec as a bundle. Be precise --
   cite line:line refs from the diff doc.
4. Run pytest tests/test_layout/ -x --tb=short -q -k "fmmm" (or
...
7. If COMMITTED: `feat(fidelity): round 22 fmmm -- <short fix description>`
8. If criterion missed: revert. Write `ROUND_22_RESIDUAL_fmmm.md`.
9. Per-round summary: `eval_output/algo_fidelity/round_22/fmmm/SUMMARY.md`

## Scope

**Allowed**:
- The dagua ops/pipeline files for fmmm (located via the ROUND_21_DIFF doc's "Files Read" section)
...
- `eval_output/algo_fidelity/round_22/fmmm/**`
- `.project-context/research/sprint_algo_fidelity/ROUND_22_*fmmm*.md`
- `tests/test_layout/test_*fmmm*.py` for regression tests + snapshot updates
...
- Any other family's pipeline/ops files (you only own fmmm)
...
<scope_constraints>fmmm family files only. NO other family code.</scope_constraints>
