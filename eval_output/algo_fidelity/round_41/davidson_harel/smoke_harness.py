"""Round 41 Davidson-Harel fidelity smoke harness."""
...
from dagua.eval.competitors.igraph_competitor import IgraphDavidsonHarel  # noqa: E402
...
_OUTPUT_DIR = Path("eval_output/algo_fidelity/round_41/davidson_harel")
_ROUNDS = 1
_SEEDS = (0, 1, 2)
...
def _reference_positions(edge_index: torch.Tensor, num_nodes: int, seed: int) -> torch.Tensor:
    """Run the igraph Davidson-Harel reference adapter.
    ...
    result = IgraphDavidsonHarel().layout_with_variant(
        graph,
        seed=seed,
        variant_params={"maxiter": _ROUNDS},
    )
    if result.pos is None:
        raise RuntimeError(f"igraph reference failed: {result.error}")
    return result.pos


def _rmsd(candidate: torch.Tensor, reference: torch.Tensor) -> float:
    """Compute Procrustes RMSD for one layout pair.
...
def main() -> None:
    ...
    for topology, builder in topologies.items():
        ...
        for seed in _SEEDS:
            ...
            before = layout_davidson_harel_pipeline(
            ...
            rows.append(
                {
                    ...
                    "max_abs_after": float(torch.max(torch.abs(after - reference)).item()),
                }
            )
    ...
    with (_OUTPUT_DIR / "smoke_rmsd.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=["topology", "seed", "before_rmsd", "after_rmsd", "max_abs_after"],
        )
        writer.writeheader()
        writer.writerows(rows)

    mean_before = statistics.fmean(float(row["before_rmsd"]) for row in rows)
    mean_after = statistics.fmean(float(row["after_rmsd"]) for row in rows)
    print(f"rows={len(rows)} mean_before={mean_before:.6f} mean_after={mean_after:.6f}")
